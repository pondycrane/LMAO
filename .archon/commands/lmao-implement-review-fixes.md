---
description: Implement CRITICAL and HIGH fixes from review, add tests, report remaining issues. LMAO-adapted: Bazel validation, hardware safety.
...
- Do NOT remove Bazel runfiles workarounds (`sys.path` bootstraps, `imports = ["."]`, "Bazel sandbox" comments) — tests fail at module load without them (issue #87).
...
3. **Verify fix builds**: \`bazel build //...\` + \`ruff check <changed files>\`
...
4. **Verify tests pass**: \`bazel test //tests:<target> --test_output=errors\`
...
Must pass. If not, fix build errors.
...
Fix any lint/type errors introduced.
