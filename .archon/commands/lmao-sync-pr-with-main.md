---
description: Sync PR branch with latest main (rebase if needed, resolve conflicts if any). LMAO-adapted: Bazel validation.
...
- **Bazel is the canonical build.** Validate with `bazel build //...` + `bazel test //tests:all --test_tag_filters=-requires_hardware` + `ruff`/`mypy` on changed files.
- Do NOT remove Bazel runfiles workarounds (`sys.path` bootstraps, `imports = ["."]`, "Bazel sandbox" comments) while resolving conflicts (issue #87).
...
Validation: \u2705 Bazel build | \u2705 Tests | \u2705 ruff/mypy
