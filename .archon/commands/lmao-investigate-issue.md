---
description: Investigate a GitHub issue or problem - analyze codebase, create plan, post to GitHub. LMAO-adapted: Bazel validation commands, hardware safety rules.
...
(New test files MUST get a `py_test` target in `tests/BUILD` — otherwise they never run, issue #87.)

### Manual Verification

1. {Step to verify the fix/feature works}
2. {Step to verify no regression}
3. {If client/server/protocol touched: hardware E2E per AGENTS.md — `bazel test //tests:test_cardputer_e2e //tests:test_cardputer_lora_e2e --test_output=all`}
...
\`\`\`bash
bazel build //... && bazel test //tests:all --test_tag_filters=-requires_hardware --test_output=errors && ruff check <changed-files> && mypy <changed-files>
\`\`\`
