---
description: Create comprehensive feature implementation plan with codebase analysis and research. LMAO-adapted: Bazel validation commands, hardware safety rules.
...
**Hardware safety — plans must respect these; violating them can brick physical devices:**

1. **NEVER run esptool on the Cardputer** (`/dev/ttyACM*`) — the Cardputer is flashed ONLY via `bazel run //cardputer_client:flash` (MicroPython raw REPL).
2. **NEVER flash the RNode via esptool** (`/dev/ttyUSB*`) — the only supported method is the web flasher at https://flasher.rnode.network/.
3. **Leave the production Cardputer running** — any plan touching the client must end with the device running production config (server `DEST_HASH`) again.
4. Radio parameters (868 MHz, BW 125 kHz, SF 7, CR 4:5, preamble 24, syncword 0x1424) must stay in sync between server and client.

**Build rules:**

- **Bazel is the canonical build** (v7.4.1 via bazelisk). Every new `*.py` file in the plan MUST include a BUILD target change — a test file without a `py_test` target silently never runs (issue #87).
- Lint: `ruff check` / `ruff format --check` (config `ruff.toml`). Type-check: `mypy` (config `mypy.ini`). Vendored code (`cardputer_client/lib/urns/**`, `rnode_firmware/esptool.py`) is lint-exempt.
- Unit gate: `bazel test //tests:all --test_tag_filters=-requires_hardware`. Hardware E2E (when devices attached): `bazel test //tests:test_cardputer_e2e //tests:test_cardputer_lora_e2e --test_output=all`.
- Dependencies are pinned in `lmao_server/requirements_lock.txt` — catalog versions from there, not package.json (there is none).
...
cat lmao_server/requirements.txt 2>/dev/null | head -20
...
- [ ] Dependencies cataloged with versions from lmao_server/requirements_lock.txt
...
- Official documentation for involved libraries (match versions from lmao_server/requirements_lock.txt)
...
- [ ] Documentation versions match lmao_server/requirements_lock.txt
...
\`\`\`bash
bazel test //tests:<relevant-targets> --test_output=errors
\`\`\`

**EXPECT**: All tests pass. (New test files MUST get a `py_test` target in `tests/BUILD` — otherwise they never run, issue #87.)
...
**EXPECT**: All tests pass, build succeeds. (Hardware E2E — `bazel test //tests:test_cardputer_e2e //tests:test_cardputer_lora_e2e --test_output=all` — runs in the workflow's hardware gate when devices are attached.)
...
- [ ] External docs versioned to match lmao_server/requirements_lock.txt
- [ ] Integration points mapped with specific file paths
