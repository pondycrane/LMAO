---
description: Aggressively fix all review findings - lean towards fixing unless clearly a new concern. LMAO-adapted: Bazel validation, hardware safety.
...
1. **NEVER run esptool on the Cardputer** (`/dev/ttyACM*`) — no flashing, no `chip_id`, no probing. Flash ONLY via `bazel run //cardputer_client:flash`.
2. **NEVER flash the RNode via esptool** (`/dev/ttyUSB*`) — web flasher (https://flasher.rnode.network/) is the only supported method.
3. **Leave the production Cardputer running** with restored config/DEST_HASH after any test session.

**Build rules:**

- **Bazel is the canonical build.** Every new `*.py` file MUST belong to a BUILD target — a test file without a `py_test` target silently never runs (issue #87). New tests in `tests/` need a `py_test` mirroring siblings (`conftest.py` in `srcs`, `imports = ["."]` where used).
- Lint: `ruff check` / `ruff format --check`; type-check: `mypy` — run on changed files only.
...
- Do NOT remove Bazel runfiles workarounds (`sys.path` bootstraps, `imports = ["."]`, "Bazel sandbox" comments) — they look redundant but the test targets fail without them (issue #87).
...
3. Verify after each fix: \`bazel build //...\` and \`ruff check <changed files>\`
...
3. **Wire new test files into `tests/BUILD`** — a test without a `py_test` target never runs (issue #87)
4. Run them: \`bazel test //tests:<target> --test_output=errors\`
...
- [ ] Unit tests pass
- [ ] New `.py` files belong to a BUILD target
...
\u2705 Bazel build | \u2705 ruff/mypy | \u2705 Unit tests ({n} passed)

---

*Self-fix by Archon \xB7 aggressive mode \xB7 fixes pushed to \`{HEAD_BRANCH}\`*
EOF
)"
\`\`\`
